...
static inline Obj* objNext(Obj* object) {
  return (Obj*)(uintptr_t)(object->header & 0x0000ffffffffffffULL);
}

static inline void setIsMarked(Obj* object, bool marked) {
  object->header = (object->header & 0xff00ffffffffffffULL) |
      ((uint64_t)marked << 48);
}

static inline void setObjNext(Obj* object, Obj* next) {
  object->header = (object->header & 0xffff000000000000ULL) |
      (uint64_t)(uintptr_t)next;
}
